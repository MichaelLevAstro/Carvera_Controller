...
def parse(gcode):
    ...
    params = {letter.upper(): float(value) for letter, value in _PARAM_RE.findall(gcode[m.end() :])}
...
def preview_text(gcode):
    ...
    if RUNNER_MCODES.get(mcode) == "angle":
        return (
            "Angle: probe two points along an edge and report the measured "
...
def bore_preview_text(gcode):
    ...
    return (
        "Bore centre (multi-point): probe %d points around the bore, fit a "
...
def fit_circle(points):
    ...
    for x, y in points:
        ...
        Szx += z * x
        Szy += z * y
    sol = _solve3([[Sxx, Sxy, Sx], [Sxy, Syy, Sy], [Sx, Sy, float(n)]], [-Szx, -Szy, -Sz])
    if sol is None:
        return None
    a, b, c = sol
    cx, cy = -a / 2.0, -b / 2.0
...
# ---------------------------------------------------------------------------
# Live runners (bore centre, angle)
# ---------------------------------------------------------------------------
class Z1ProbeRunner:
    ...
    def run_bore_center(self):
        ...
        self._info("Bore centre set: diameter %.3f mm, roundness %.3f mm (%d points)" % (2 * radius, 2 * roundness, n))
